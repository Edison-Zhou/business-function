...
/**
  ...
  */
object DetailVipRecommend extends BaseClass {
  ...
  override def execute(args: Array[String]): Unit = {
    ...
    //获取评分节目
    var scoreSid = BizUtils.getSearchVideoData(ss,date)(true)

    if(scoreSid.count() == 0){
      date = new SimpleDateFormat("yyyyMMdd").format(DateUtils.addDays(new Date(),-2))
      scoreSid = BizUtils.getSearchVideoData(ss,date)(true)
    }
    ...
    contentTypeList.foreach(f = contentType => {
      ...
      //转换为虚拟id
      val virSidResult = BizUtils.transferToVirtualSid(scoreSidTopN, "sid").orderBy(expr("video_score").desc).limit(300)
      ...
      val similarMix = DataReader.read(BizUtils.getHdfsPathForRead("similarMix/" + contentType))
          .select(expr("sid"),expr("item"),(expr("similarity")-100).as("score"))
      ...
      algorithm.run()
      //输出到HDFS
      algorithm.getOutputModel.output("detailVip/" + contentType)
