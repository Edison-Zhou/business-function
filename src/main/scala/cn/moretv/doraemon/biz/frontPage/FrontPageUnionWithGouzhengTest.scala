...
/**
  ...
  */
object FrontPageUnionWithGouzhengTest extends BaseClass {
  ...
  override def execute(args: Array[String]): Unit = {
    ...
    val gouzhengFullUser = DataReader.read(BizUtils.getHdfsPathForRead("gouzheng/userID"))
      .map(r=> TransformUDF.calcLongUserId(r.getString(0))).toDF("uid")
    val gouzhengUserRecWithALS = gouzhengFullUser.as("a").
      join(homePageGouzhengDF.select("uid").as("b"), expr("a.uid = b.uid"), "left").
      filter("b.uid is null").selectExpr("cast(a.uid as string) as uid")
    BizUtils.getDataFrameInfo(gouzhengUserRecWithALS, "gouzhengUserRecWithALS")
...
//      selectExpr("cast(gouzhengUserWithALS as string) as uid")
    ...
    val ALSNewAlgFoldDF = vipFoldDF.join(gouzhengUserRecWithALS, "uid")
    BizUtils.getDataFrameInfo(ALSNewAlgFoldDF, "ALSNewAlgFoldDF")

    val GZAlgFoldDF = homePageGouzhengDF.selectExpr("cast(uid as string) as uid", "id")
    BizUtils.getDataFrameInfo(GZAlgFoldDF, "GZAlgFoldDF")

    val vipWithoutGouzhengFoldDF = vipFoldDF.as("a").join(gouzhengFullUser.selectExpr("cast(uid as string) as uid").
      as("b"), expr("a.uid = b.uid"), "left").filter("b.uid is null").selectExpr("a.uid", "a.id")
...
//
    ...
    val unionDF = portalPackDF.as("a").join(vipPackDF.as("b"), expr("a.key = b.key"), "full")
      .join(hotPackDF.as("c"), expr("a.key = c.key"), "full")
