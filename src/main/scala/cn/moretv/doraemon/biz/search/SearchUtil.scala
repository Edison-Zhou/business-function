...
import cn.moretv.doraemon.biz.util.{ConfigUtil, ElasticSearchUtil, ElasticSearchUtil2}
...
object SearchUtil{
  ...
  def outputBatch4AW2Sid(df: DataFrame,
                  indexName: String,
                  redisKeyPrefix: String,
                  biz: String,
                  alg: String)
    ...
    df.foreachPartition(iter => {
      ...
      while (iter.hasNext) {
        ...
        while (size < BATCH_SIZE && iter.hasNext) {
          ...
          val row = iter.next()
          val mutableValueMap = mutable.HashMap[String, Any]()
          val searchKey = row.getAs[String]("associativeWord")
          mutableValueMap.put("key", searchKey)
          mutableValueMap.put("alg", alg)
          mutableValueMap.put("biz", biz)
          val value = row.getAs[Seq[Row]]("value")
          ...
          val indexMap = Map("index" -> Map("_type" -> "zhongwen", "_index" -> indexName, "_id" -> searchKey))
          val indexString = mapper.writeValueAsString(indexMap)

          stringBuilder.append(indexString).append("\n").append(valueString).append("\n")

          if (searchKey.length <= 3) {
            redisWriter.writeOne(Map("key" -> (redisKeyPrefix + searchKey), "value" -> mutableValueMap.toMap), getRedisPath4Zhongwen())
          }
        }
        println("Batch " + batch + " Insert --" + new Date())
        ElasticSearchUtil.bulkCreateIndex(stringBuilder.toString())
  ...
  /**
    * 将联想词相关的dataframe信息批量插入ES和redis
    * @param df       输入数据，包含字段 searchKey，contentType, content
    * @param indexName    索引名称。索引type为automated
    * @param redisKeyPrefix
    */
  def outputBatch4SK2AW(df: DataFrame,
                  indexName: String,
                  redisKeyPrefix: String,
                  biz: String,
                  alg: String)
    ...
    df.foreachPartition(iter => {
      ...
      while (iter.hasNext) {
        ...
        while (size < BATCH_SIZE && iter.hasNext) {
          ...
          mutableValueMap.put("associativeWord", value)

          val valueString = mapper.writeValueAsString(mutableValueMap.toMap)

          val indexMap = Map("index" -> Map("_type" -> "automated", "_index" -> indexName, "_id" -> searchKey))
          val indexString = mapper.writeValueAsString(indexMap)

          stringBuilder.append(indexString).append("\n").append(valueString).append("\n")

          if (searchKey.length <= 3) {
            redisWriter.writeOne(Map("key" -> (redisKeyPrefix + searchKey), "value" -> mutableValueMap.toMap), getRedisPath4AssociativeWord())
          }
        }
        println("Batch " + batch + " Insert --" + new Date())
        ElasticSearchUtil.bulkCreateIndex(stringBuilder.toString())
  ...
  var redisPath4AssociativeWord: RedisPath = _
  def getRedisPath4AssociativeWord() (implicit env: EnvEnum.Value): RedisPath = {
    if (redisPath4AssociativeWord == null) {
      val topic = ConfigUtil.get("search.redis.topic")
      val host = ConfigUtil.get("search.redis.host")
      val port = ConfigUtil.getInt("search.redis.port")
      val dbIndex = ConfigUtil.getInt("search.associativeword.redis.dbindex")
      val ttl = ConfigUtil.getInt("search.redis.ttl")
      val formatType = FormatTypeEnum.HASH
      redisPath4AssociativeWord = RedisPath(topic, host, port, dbIndex, ttl, formatType)
    }
    redisPath4AssociativeWord
  }

  var redisPath4Zhongwen: RedisPath = _
  def getRedisPath4Zhongwen() (implicit env: EnvEnum.Value): RedisPath = {
    if (redisPath4Zhongwen == null) {
      val topic = ConfigUtil.get("search.redis.topic")
      val host = ConfigUtil.get("search.redis.host")
      val port = ConfigUtil.getInt("search.redis.port")
      val dbIndex = ConfigUtil.getInt("search.zhongwen.redis.dbindex")
      val ttl = ConfigUtil.getInt("search.redis.ttl")
      val formatType = FormatTypeEnum.HASH
      redisPath4Zhongwen = RedisPath(topic, host, port, dbIndex, ttl, formatType)
    }
    redisPath4Zhongwen
  }
}
