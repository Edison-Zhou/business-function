...
import cn.mapping.SidMappingABHelios.{getProperty, getUrl}
...
object SidMappingBAHelios {
  def main(args: Array[String]): Unit = {
    ...
    val sql = "TRUNCATE TABLE sid_mapping_b_a_helios"
   ...
   ss.sql("select B.* from  B left join  A on  A.sidA = B.sidB and  A.titleA = B.titleB and A.contentTypeA=B.contentTypeB where A.sidA is   null ").createOrReplaceTempView("B")
    ...
    resultDF1.union(resultDF2).union(resultDF3).foreachPartition(par=>{
      val insertSql = "insert into sid_mapping_b_a_helios(sidA,idA,sidB,idB,contentType) values(?,?,?,?,?) "
      val db = MySqlOps(sidMappingProp)
      par.foreach(row=>{
