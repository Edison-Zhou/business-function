...
import cn.mapping.SidMappingAB.{getProperty, getUrl}
...
object SidMappingBA {
  def main(args: Array[String]): Unit = {
    ...
    val sql = "TRUNCATE TABLE sid_mapping_b_a"
    val db = MySqlOps(sidMappingProp)
    //清空记录
    db.truncate(sql)
    db.destory()
    //现网数据库
    val proUrl = getUrl("10.255.130.4","tvservice")
    val proProp = getProperty(proUrl,"bislave","slave4bi@whaley")


    //测试网数据库
    val testUrl = getUrl("10.10.147.210","tvservice")
    val testProp = getProperty(testUrl,"readonly","readonly")

    getDateFrame(ss,testProp,testUrl,"test").dropDuplicates(Array("sidB")).createOrReplaceTempView("B")

    val proDF = getDateFrame(ss,proProp,proUrl,"pro").persist(StorageLevel.MEMORY_AND_DISK)

    //以
    proDF.dropDuplicates(Array("sidA","titleA","contentTypeA")).createOrReplaceTempView("A")

    val resultDF1 = ss.sql("select A.sidA,A.idA,A.contentTypeA,B.sidB,B.idB from  B left join  A on  A.sidA = B.sidB and  A.titleA = B.titleB and A.contentTypeA=B.contentTypeB where A.sidA is not null ")
   ss.sql("select B.* from  B left join  A on  A.sidA = B.sidB and  A.titleA = B.titleB and A.contentTypeA=B.contentTypeB where A.sidA is   null ").createOrReplaceTempView("B")



    proDF.dropDuplicates(Array("titleA","contentTypeA")).createOrReplaceTempView("A")
    val resultDF2 = ss.sql("select A.sidA,A.idA,A.contentTypeA,B.sidB,B.idB from  B left join  A on   A.titleA = B.titleB and A.contentTypeA=B.contentTypeB where A.sidA is not null ")
    ss.sql("select B.* from  B left join  A on    A.titleA = B.titleB and A.contentTypeA=B.contentTypeB where A.sidA is  null ").createOrReplaceTempView("B")


    proDF.dropDuplicates(Array("contentTypeA")).createOrReplaceTempView("A")
    val resultDF3 = ss.sql("select A.sidA,A.idA,A.contentTypeA,B.sidB,B.idB from B  left join  A on   A.contentTypeA=B.contentTypeB where A.sidA is not null ")

    resultDF1.union(resultDF2).union(resultDF3).foreachPartition(par=>{
      val insertSql = "insert into sid_mapping_b_a(sidA,idA,sidB,idB,contentType) values(?,?,?,?,?) "
      val db = MySqlOps(sidMappingProp)
      par.foreach(row=>{
