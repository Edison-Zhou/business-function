...
import cn.moretv.doraemon.algorithm.longVideoCluster. {Cluster4OfflineIndicatorAlgorithm,Cluster4OfflineIndicatorParameters}
...
/**
  ...
  */
object HalfMonthAgoCluster extends BaseClass {
  ...
  override def execute(args: Array[String]): Unit = {
    try {
      ...
      //算法部分
      val longVideoClusterAlgorithm = new Cluster4OfflineIndicatorAlgorithm
      val longVideoClusterParameters = longVideoClusterAlgorithm.getParameters.asInstanceOf[Cluster4OfflineIndicatorParameters]
      ...
      //输出目录为："/ai/output/medusa/longVideoClusterRecommend/Latest"
      longVideoClusterAlgorithm.getOutputModel.output("HalfMonthAgoCluster")
    } catch {
      case e: Exception => {
        throw e
      }
