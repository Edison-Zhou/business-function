...
/**
  ...
  */
object WideAndDeep extends BaseClass{
  ...
  override def execute(args: Array[String]): Unit = {
    ...
    val modelPath = "/ai/tmp/WideAndDeepModelEpoch2_tag"
    val predictionPath = "PSWideAndDeepPredictEpoch2_tag"
    ...
    SharedConf.get().setDouble(MLConf.ML_BATCH_SAMPLE_RATIO, 0.02)
    SharedConf.get().setLong(MLConf.ML_FIELD_NUM, 128)
    SharedConf.get().setLong(MLConf.ML_RANK_NUM, 64)

    SharedConf.get().set(AngelConf.ANGEL_RUNNING_MODE, RunningMode.ANGEL_PS.toString)

    val model = new WideAndDeepPS
