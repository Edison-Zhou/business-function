...
/**
  ...
  * @since 2019/6/25
  */
object RecallNew extends BaseClass {
  ...
  val numOfHotVideo = 50
  //从数据中获取的豆瓣高评分节目数量
  val numOfHighScore = 20
  //分数阈值设定为8，在离线指标中中发现有8.3分数的影片召回遗漏次数多
  ...
  override def execute(args: Array[String]): Unit = {
    ...
    //获得als推荐数据
    val alsRecommendMovie = BizUtils.getUidSidDataFrame(DataReader.read(BizUtils.getHdfsPathForRead("ALS/rec4movie")),
      110, Constants.ARRAY_OPERATION_TAKE_AND_RANDOM).repartition(1000).join(validMovie, "sid")
      .as("a").join(activeUser.as("b"), expr("a.uid =b.user")).selectExpr("a.uid as uid", "a.sid as sid")
    //获得看过的最后一部电影的相似内容推荐
    val LastMoviesimilarRecommend = BizUtils.getUidSidDataFrame(DataReader.read(new HdfsPath("/ai/tmp/output/pre/medusa/similarityRecommend/Latest")),
    ...
    //获得长视频聚类推荐
    val MovieClusterRecommend = BizUtils.getUidSidDataFrame(DataReader.read(new HdfsPath("/ai/tmp/output/pre/medusa/longVideoClusterRecommend/Latest")),
      -1, Constants.ARRAY_OPERATION_RANDOM).repartition(1000).join(validMovie, "sid")
      .as("a").join(activeUser.as("b"), expr("a.uid =b.user")).selectExpr("a.uid as uid", "a.sid as sid")
    ...
    //获取用户前一个月观看的电影信息
    val startDate = DateUtils.farthestDayWithDelimiter(60)
    val endDate = DateUtils.farthestDayWithDelimiter(31)
    ...
    val userWatchedMovieTrunc = BizUtils.rowNumber(userWatchedMovie, "uid",
      "score", numOflastMonthWatched, true)
    ...
    val searchSimilarMovie = searchRes.as("a").join(DataReader.read(BizUtils.getHdfsPathForRead("similarMix/movie"))
    ...
    mixAlg.initInputData(Map(mixAlg.INPUT_DATA_KEY_PREFIX + "1" -> MovieClusterRecommend,
      ...
      mixAlg.INPUT_DATA_KEY_PREFIX + "3" -> LastMoviesimilarRecommend, mixAlg.INPUT_DATA_KEY_PREFIX + "4" -> alsRecommendMovie))
    mixAlg.run()
    ...
    val clusterWithSource = MovieClusterRecommend.withColumn("source", lit("userWatched"))
    ...
    val userWatchedLongVideos = BizUtils.getUserWatchedLongVideo(30)
    val filteredLongVideos = frontPageExposedLongVideos.union(userWatchedLongVideos)
    BizUtils.getDataFrameInfo(filteredLongVideos, "filteredLongVideos")
    val recallUidSid = BizUtils.uidSidFilter(unionDF, filteredLongVideos,"left","black")
...
/*
    ...
    BizUtils.outputWrite(recallRes, "recall/movie")
    BizUtils.outputWrite(recallResWithSource, "recall/sourceTrack")
